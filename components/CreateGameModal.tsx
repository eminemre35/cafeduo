         ...
         <form onSubmit={handleSubmit} className="space-y-6">
           ...
           <div>
             <div className="flex justify-between items-center mb-2">
                <label className="block text-blue-400 font-pixel text-xs tracking-widest">BAHİS PUANI</label>
                <span className="font-mono text-xs text-gray-500">BAKİYE: {maxPoints}</span>
             </div>
             ...
             <div className="relative">
                <input 
                    ...
                    className="w-full bg-black border-2 border-gray-600 text-white p-3 font-retro text-xl focus:border-blue-500 outline-none transition-colors text-center"
                />
             </div>
             <input 
                type="range" 
                ...
                value={points} 
                onChange={(e) => setPoints(Number(e.target.value))}
                className="w-full mt-4 accent-blue-500 h-2 bg-gray-800 rounded-lg appearance-none cursor-pointer"
             />
           </div>
